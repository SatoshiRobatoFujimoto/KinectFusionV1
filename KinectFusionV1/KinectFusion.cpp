	...
	while (1){
		...
		Concurrency::parallel_for(0, height, [&](int y){
			for (int x = 0; x < width; x++){
				...
				if (pDepthPlayerPixel[index].depth != 0 && (int)pDepthPlayerPixel[index].depth < 4500 
					&& 0 < pColorPoint[index].y * width + pColorPoint[index].x && pColorPoint[index].y * width + pColorPoint[index].x < height * width){
					pDst[index] = pSrc[pColorPoint[index].y * width + pColorPoint[index].x];
					//std::cout << pDepthPlayerPixel[index].depth << std::endl;
				}
				else{
					pDst[index] = 0;
				}
