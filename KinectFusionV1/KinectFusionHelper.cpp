...
HRESULT WriteBinarySTLMeshFile(INuiFusionColorMesh *mesh, char* lpOleFileName, bool flipYZ)
{
	HRESULT hr = S_OK;
...
HRESULT WriteAsciiObjMeshFile(INuiFusionColorMesh *mesh, char* lpOleFileName, bool flipYZ)
{
	HRESULT hr = S_OK;
...
HRESULT WriteAsciiPlyMeshFile(INuiFusionColorMesh *mesh, char* lpOleFileName, bool flipYZ, bool outputColor)
{
	HRESULT hr = S_OK;
...
HRESULT WriteTexturedeAsciiObjMeshFile(INuiFusionColorMesh *mesh, char* lpOleFileName, bool flipYZ, NUI_FUSION_IMAGE_FRAME *pTexture, const std::vector<Vector3> &texcoords)
{
	HRESULT hr = S_OK;
	...
	int length = MultiByteToWideChar(CP_ACP, 0, textureFilenamePath.c_str(), static_cast<int>(textureFilenamePath.length()) + 1, 0, 0);
	wchar_t* pszW = new wchar_t[length];
	MultiByteToWideChar(CP_ACP, 0, textureFilenamePath.c_str(), static_cast<int>(textureFilenamePath.length()) + 1, pszW, length);
